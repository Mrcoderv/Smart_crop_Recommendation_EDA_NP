...
tab1, tab2, tab3 = st.tabs(["🤖 AI Prediction (ML)", "📊 Rule-Based", "📈 Analytics"])
...
with tab1:
    ...
    if not model_status['available']:
        ...
        st.info("""
        The ML model needs to be trained first. Run the following command in your terminal:
        ```bash
...
with tab2:
    ...
    selected_crop = st.selectbox("Select a crop to view details", get_all_crops(), key="detail_crop")
...
# ============================================================================
with tab3:
    st.markdown("### 📈 ML Model Analytics & Information")
    ...
    with col2:
        st.metric("Input Features", "6 variables")
    
    with col3:
        st.metric("Crop Classes", "10 crops")
    
    st.markdown("---")
    
    # Check if model is available for analytics
    if check_model_availability():
        ...
        if ml_predictor:
            ...
            display_feature_importance(ml_predictor)
            ...
            **Machine Learning Model Details:**
            ...
            - **Training Data:** Crop recommendation dataset (100 samples)
            - **Features:** Temperature, Humidity, Rainfall, Nitrogen, Phosphorus, Potassium
            - **Target:** 10 Crop types (Rice, Wheat, Maize, Potato, Tomato, Onion, Cabbage, Millet, Barley, Lentil)
            
            **Model Performance:**
            - Evaluation metrics are calculated during training
            - Check `models/training_report.txt` for detailed metrics
            
            **How It Works:**
            1. Takes 6 environmental parameters as input
            2. Scales features using StandardScaler
            3. Predicts using Random Forest (100 trees)
            4. Returns confidence scores for all crops
            5. Highlights top 3 recommendations
            """)
    else:
        st.warning("⚠️ Model not trained. Please run: `python models/train_model.py`")
